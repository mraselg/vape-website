...
<header class="site-header" id="siteHeader">
  <div class="container header-main">
    ...
    <a class="brand" href="#top" aria-label="<?= e(trim(($S['brand_name'] ?? '') . ' ' . ($S['brand_tagline'] ?? ''))) ?> home">
    ...
    <div class="header-actions">
      <button class="icon-btn theme-header-btn" id="headerThemeToggle" aria-label="Toggle dark/light mode" title="Toggle theme">
  ...
  <nav class="header-nav container" aria-label="Categories">
    <ul>
      ...
      <li><a href="index.php#bestsellers" class="hot">Best Selling Products</a></li>
    </ul>
  </nav>
</header>
...
<aside class="menu-drawer" id="menuDrawer" aria-label="Menu" aria-hidden="true">
  <div class="menu-drawer-head">
    <span class="brand-text"><strong><?= e($S['brand_name'] ?? 'VAPE CLUB') ?></strong> <small><?= e(strtoupper($S['brand_tagline'] ?? '')) ?></small></span>
    <button class="icon-btn" id="menuClose" aria-label="Close menu"><svg class="icon"><use href="#i-close"/></svg></button>
  </div>
  <nav>
    <a href="index.php#shop">🏠 All Products</a>
    <a href="index.php#bestsellers">🏆 Best Selling Products</a>
    <a href="category.php?cat=iluma">⚡ IQOS ILUMA Devices</a>
    <a href="category.php?cat=terea-id"><span class="flag">🇮🇩</span> TEREA Indonesia</a>
    <a href="category.php?cat=terea-jp"><span class="flag">🇯🇵</span> TEREA Japan</a>
    <a href="category.php?cat=terea-ch"><span class="flag">🇨🇭</span> TEREA Swiss</a>
    <a href="category.php?cat=disposables">🔥 Disposables 10k+ Puffs</a>
    <a href="category.php?cat=pod">💠 Pod Systems</a>
    <a href="category.php?cat=eliquid">💧 E-Liquids</a>
  </nav>
  ...
  <div class="menu-contact">
    <a class="btn btn-wa btn-block" href="<?= e(wa_link($S['wa_msg_order'] ?? 'Hello! I want to order.')) ?>" target="_blank" rel="noopener">
      <svg class="icon"><use href="#i-wa"/></svg> WhatsApp Order
    </a>
    <a class="btn btn-ghost btn-block" href="tel:<?= e($S['phone_tel'] ?? '') ?>">
      <svg class="icon"><use href="#i-phone"/></svg> <?= e($S['phone_display'] ?? '') ?>
    </a>
  </div>
  <p class="menu-note">18+ only · ESMA compliant · Delivery across UAE</p>
</aside>
